...
public class ActionSpeedPlugin : BaseUnityPlugin {
	...
	private void Awake() {
		...
		try {
			...
			m_font_scale_factor = this.Config.Bind<float>("General", "Font Size Multiplier", 1.25f, "Float value multiplied times base font size to increase/decrease all text size.");
			if (m_enabled.Value) {
