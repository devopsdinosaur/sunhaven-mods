...
public class ExpandedStoragePlugin : BaseUnityPlugin {
	...
	class HarmonyPatch_Chest_LoadInventory {

		private static bool Prefix(Inventory __instance, Transform ____inventoryPanel) {
			try {
				if (!m_enabled.Value) {
					return true;
				}
				...
				bool find_slots(Transform transform) {
					...
					if (slot_index == 0) {
						...
						for (;;) {
							...
							x = 0;
							y = 0;
							for (int check_index = 0; check_index < m_num_chest_slots.Value; check_index++) {
				...
				logger.LogError("** Inventory.Start_Prefix ERROR - " + e);
			}
			return true;
		}
	}
